			...
			<div class="bg-white rounded-lg shadow-lg max-w-6xl mx-auto">
				...
				<div class="px-6 pt-2 pb-6">
					...
					<thead class="bg-gray-100 text-gray-900 font-semibold">
						...
						<td class="px-4 py-2 cursor-pointer">Code</td>
					...
					<tbody class="divide-y divide-gray-200">
						...
						>
						<td class="px-4 py-2 align-top w-5">{{ dept.code }}</td>
						<td class="px-4 py-2 align-top">{{ dept.name }}</td>
                    ...
                    <div class="bg-white rounded-lg w-[600px] absolute top-32">
                        ...
                        <div class="flex flex-col gap-3 px-6 py-4">
                            <div class="flex flex-col gap-1">
                                <label class="text-sm" for=""> Department Name</label>
                                <input placeholder="Item Code" class="border px-3 py-2 rounded text-sm"/>
                            </div>
                            <div class="flex flex-col gap-1">
                                <label class="text-sm" for=""> Department Name</label>
                                <input placeholder="Item Code" class="border px-3 py-2 rounded text-sm"/>
                        ...
                        <div class="flex justify-end gap-2 px-6 py-4">
                            ...
                            <button @click="saveDept" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{{ isEdit ? 'Update' : 'Add' }}</button>
                        </div>
                    </div>
				</div>
...
const depts = reactive([
  { id: 1, name: "IT Department", code: "ITB" },
  { id: 2, name: "Finance Department", code: "FNC" },
  { id: 3, name: "Purchasing Department", code: "PUR" },
]);
...
const saveDept = () => {
  if (isEdit.value) {
    const index = depts.findIndex(i => i.id === modalItem.id);
    if (index !== -1) depts[index] = { ...modalItem };
  } else {
    depts.push({ ...modalItem, id: Date.now() });
  }
...
const deleteItem = () => {
  const index = depts.findIndex(i => i.id === modalItem.id);
  if (index !== -1) depts.splice(index, 1);
