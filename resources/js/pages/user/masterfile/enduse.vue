		...
		<section class="items-center justify-center py-8">
			<div class="bg-white rounded-lg shadow-lg max-w-6xl mx-auto">
              ...
              <tbody class="divide-y divide-gray-200">
                  <tr
                  v-for="enduse in enduses"
                  :key="enduse.id"
                  class="hover:bg-gray-50"
                  >
                  <td class="px-4 py-2 align-top">{{ enduse.name }}</td>
                ...
                <div class="bg-white rounded-lg w-[600px] absolute top-32">
                    ...
                    <div class="flex flex-col gap-3 px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <label class="text-sm" for=""> Enduse</label>
                            <input placeholder="Enduse" class="border px-3 py-2 rounded text-sm"/>
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 px-6 py-4">
                        <button @click="closeModal" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-400">Cancel</button>
                        <button @click="saveEnduse" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{{ isEdit ? 'Update' : 'Add' }}</button>
                    </div>
                </div>
            </div>
...
// Table data
const enduses = reactive([
  { id: 1, name: "Production / Manufacturing" },
...
const saveEnduse = () => {
  if (isEdit.value) {
    const index = enduses.findIndex(i => i.id === modalItem.id);
    if (index !== -1) enduses[index] = { ...modalItem };
  } else {
    enduses.push({ ...modalItem, id: Date.now() });
  }
...
const deleteItem = () => {
  const index = enduses.findIndex(i => i.id === modalItem.id);
  if (index !== -1) enduses.splice(index, 1);
