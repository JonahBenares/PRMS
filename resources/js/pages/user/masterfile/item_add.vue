	...
	<navigation>
		<pageCard>
			...
			<div class="bg-gray-50 border rounded-xl p-5 mb-6">
				...
				<div class="grid md:grid-cols-3 gap-4">
					...
					<div>
						...
						<input type="text" :value="itemCode" readonly class="form-input bg-gray-100 font-semibold" />
					</div>

					<!-- Sub Category -->
					<div>
						<label class="form-label">Sub Category</label>
						<select v-model="selectedSubCategory" @change="onSubCategoryChange" class="form-input">
							<option :value="null">Select Sub Category</option>
							<option v-for="sub in subCategories" :key="sub.id" :value="sub">
								{{ sub.sub_cat_name }}
							</option>
						</select>
					</div>
					...
					<div>
						...
						<input :value="categoryName" readonly class="form-input bg-gray-100" />
					</div>
				</div>
				...
				<div class="mt-4">
					...
					<input v-model="itemDescription" class="form-input" placeholder="Enter item description..." />
				</div>
			</div>
			...
			<div class="space-y-5">
				<div v-for="(variant, index) in variants" :key="index" class="bg-white border rounded-xl shadow-sm">
					...
					<div class="flex justify-between items-center bg-gray-100 px-4 py-3 rounded-t-xl">
						...
						<button v-if="canDeleteVariant(variant)" @click="deleteVariant(index)"
					...
					<div class="px-4 pb-5">
						...
						<div class="grid grid-cols-3 gap-4">
							<div v-for="(image, imgIndex) in variant.images" :key="imgIndex" class="upload-tile">
								...
								<button v-if="image.url" @click.stop="removeImage(variant, imgIndex)"
									class="upload-remove">
									✕
								</button>
							</div>
			...
			<div class="sticky bottom-0 bg-white border-t mt-8 pt-4 flex justify-end">
				<button @click="saveItem" class="btn-success">
					Save Item
				</button>
			</div>
