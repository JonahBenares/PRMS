		...
		<section class="items-center justify-center py-8">
			<div class="bg-white rounded-lg shadow-lg max-w-6xl mx-auto">
              ...
              <thead class="bg-gray-100 text-gray-900 font-semibold">
                  <tr>
                  <td class="px-4 py-2 cursor-pointer">Qualifier Name</td>
                ...
                <div class="bg-white rounded-lg w-[600px] absolute top-32">
                    ...
                    <div class="flex flex-col gap-3 px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <label class="text-sm" for="">Qualifier</label>
                            <input v-model="modalItem.name" placeholder="Qualifier" class="border px-3 py-2 rounded text-sm"/>
                        </div>
                    </div>
...
const saveQualifier = () => {
  if (isEdit.value) {
    const index = qualifiers.findIndex(i => i.id === modalItem.id);
    if (index !== -1) qualifiers[index] = { ...modalItem };
  } else {
    qualifiers.push({ ...modalItem, id: Date.now() });
  }
...
const deleteItem = () => {
  const index = qualifiers.findIndex(i => i.id === modalItem.id);
  if (index !== -1) qualifiers.splice(index, 1);
...
onMounted(() => {
  $('#itemTable').DataTable({
    ...
    language: {
      search: "",
      searchPlaceholder: "Search qualifiers...",
