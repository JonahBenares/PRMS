		...
		<section class="items-center justify-center py-8">
			<div class="bg-white rounded-lg shadow-lg max-w-6xl mx-auto">
				...
				<div class="px-6 py-4 flex justify-between items-center mb-4 bg-white border-b rounded-t-lg">
					<h2 class="text-lg font-semibold">Warehouse List</h2>
					<button 
              ...
              <thead class="bg-gray-100 text-gray-900 font-semibold">
                  <tr>
                  <td class="px-4 py-2 cursor-pointer">Warehouse Name</td>
              ...
              <tbody class="divide-y divide-gray-200">
                  ...
                  >
                  <td class="px-4 py-2 align-top">{{ warehouse.name }}</td>
                ...
                <div class="bg-white rounded-lg w-[600px] absolute top-32">
                    ...
                    <div class="flex flex-col gap-3 px-6 py-4">
                        <div class="flex flex-col gap-1">
                            <label class="text-sm" for=""> Warehouse</label>
                            <input placeholder="Warehouse" class="border px-3 py-2 rounded text-sm"/>
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 px-6 py-4">
                        <button @click="closeModal" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-400">Cancel</button>
                        <button @click="saveWarehouse" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{{ isEdit ? 'Update' : 'Add' }}</button>
                    </div>
                </div>
            </div>
...
const warehouses = reactive([
  ...
  { id: 2, name: "IT Supplies Warehouse" },
  { id: 3, name: "Operations Warehouse" },
]);
...
const saveWarehouse = () => {
  if (isEdit.value) {
    const index = warehouses.findIndex(i => i.id === modalItem.id);
    if (index !== -1) warehouses[index] = { ...modalItem };
  } else {
    warehouses.push({ ...modalItem, id: Date.now() });
  }
