	...
	<navigation>
		<pageCard>
			...
			<div class="flex justify-between items-center mb-3">
				...
				<button @click="addVariant" class="btn-primary">
			...
			<!-- Variant Cards -->
			<div class="space-y-5">
				<div v-for="(variant, index) in variants" :key="index" class="bg-white border rounded-xl shadow-sm">
					<!-- Variant Header -->
					<div class="flex justify-between items-center bg-gray-100 px-4 py-3 rounded-t-xl">
						...
						<button v-if="variant.isNew" @click="deleteVariant(index)"
					...
					<div class="p-4 grid md:grid-cols-3 gap-4">
						...
						<div>
							...
							<input v-model="variant.partNo" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">Type</label>
							<input v-model="variant.type" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">Model</label>
							<input v-model="variant.model" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">Size</label>
							<input v-model="variant.size" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">Color</label>
							<input v-model="variant.color" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">Material</label>
							<input v-model="variant.material" class="form-input" :readonly="variant.readonly" />
						</div>

						<div>
							<label class="form-label">UOM</label>
							<input v-model="variant.uom" class="form-input" :readonly="variant.readonly" />
						</div>

						<!-- NEW FIELDS -->
						<div>
							<label class="form-label">Rating</label>
							<input class="form-input" :readonly="variant.readonly" />
						</div>
					...
					<div class="px-4 pb-4">
						...
						<textarea class="form-input" rows="3" :readonly="variant.readonly">
				</textarea>
					</div>
					...
					<div class="px-4 pb-5">
						...
						<div class="grid grid-cols-3 gap-4">
							<div v-for="(image, imgIndex) in variant.images" :key="imgIndex" class="upload-tile">
								<img v-if="image.url" :src="image.url" class="upload-img" />
								...
								<button v-if="image.url && !variant.readonly"
									@click.stop="removeImage(variant, imgIndex)" class="upload-remove">
									✕
								</button>
							</div>
						</div>
			...
			<!-- SAVE BAR -->
			<div v-if="hasNewVariants" class="sticky bottom-0 bg-white border-t mt-8 pt-4 flex justify-end">
				<button @click="saveNewVariants" class="btn-success">
					Save New Variants
				</button>
			</div>
